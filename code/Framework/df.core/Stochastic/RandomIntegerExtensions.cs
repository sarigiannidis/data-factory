...
namespace Df.Stochastic
{
    ...
    public static class RandomIntegerExtensions
    {
        ...
        public static int NextInt32(this IRandom random, int min, int max) => min + (int)((max - min) * random.NextPercentage());

        public static long NextInt64(this IRandom random) => BitConverter.ToInt64(random.NextBytes(8), 0);

        public static long NextInt64(this IRandom random, long min, long max) => min + (long)((max - min) * random.NextPercentage());
